#ifndef _FinalBDT_ttZ_vs_zjets_OS_h
#define _FinalBDT_ttZ_vs_zjets_OS_h
...
class FinalBDT_ttZ_vs_zjets_OS: public KinematicVariable<double> {
...
public:
  ...
  FinalBDT_ttZ_vs_zjets_OS(BNjetCollection **_jets,
...
FinalBDT_ttZ_vs_zjets_OS::FinalBDT_ttZ_vs_zjets_OS(BNjetCollection **_jets,
                                                   BNjetCollection **_jets_40,
                                                   TwoObjectKinematic<BNleptonCollection,BNjetCollection> * _myMTOfLepsJets,
                                                   TwoObjectKinematic<BNleptonCollection,BNjetCollection> * _myMassOfLepsJets,
                                                   BNjetCollection **_jetsByCSV,
  ...
  //std::cout << "Setting up FinalBDT_ttZ_vs_zjets_OS" << std::endl;
  
  branches["FinalBDT_ttZ_vs_zjets_OS"] = BranchInfo<double>("FinalBDT_ttZ_vs_zjets_OS");
  
  std::vector< TString >catList;
  catList.push_back("eq5j_zjets"); //0
  catList.push_back("ge6j_zjets"); //1
  ...
  for( unsigned int jj = 0 ; jj < 2 ; ++jj ) { //2 readers for 2 BDTs
    ...
    TString dir = (string(getenv("CMSSW_BASE"))+"/src/ttHMultileptonAnalysis/TemplateMakers/data/NOVa/BDT_weights_ttZ_vs_zjets_ttbar_OS/").c_str();
...
void FinalBDT_ttZ_vs_zjets_OS::evaluate() {
  if (this->evaluatedThisEvent) return;
  if ((*jets)->size() < 5) return;
  evaluatedThisEvent = true;

  //std::cout << "Inside FinalBDT_ttZ_vs_zjets_OS::evaluate()" << std::endl;
  ...
  for( unsigned int jj = 0 ; jj < 2 ; ++jj ) {
    ...
    if (jj == 0 && (*jets)->size() == 5) branches["FinalBDT_ttZ_vs_zjets_OS"].branchVal = annOut;
    if (jj == 1 && (*jets)->size() >= 6) branches["FinalBDT_ttZ_vs_zjets_OS"].branchVal = annOut;
