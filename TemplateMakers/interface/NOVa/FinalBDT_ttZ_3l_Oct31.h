#ifndef _FinalBDT_ttZ_3l_Oct31_h
#define _FinalBDT_ttZ_3l_Oct31_h
...
class FinalBDT_ttZ_3l_Oct31: public KinematicVariable<double> {
...
public:
  ...
  FinalBDT_ttZ_3l_Oct31(BNjetCollection **_jets,
...
FinalBDT_ttZ_3l_Oct31::FinalBDT_ttZ_3l_Oct31(BNjetCollection **_jets,
                                 BNjetCollection **_mediumCSVJets,
                                 MatchTester_ttZ_3l * _myMatchTester_ttZ_3l,
                                 TwoObjectKinematic<BNleptonCollection,BNleptonCollection> * _myZLikeMassLepLepSFOSAll):
  ...
  //std::cout << "Setting up FinalBDT_ttZ_3l_Oct31" << std::endl;
  
  branches["FinalBDT_ttZ_3l_Oct31"] = BranchInfo<double>("FinalBDT_ttZ_3l_Oct31");
  ...
  for( unsigned int jj = 0 ; jj < 2 ; ++jj ) { //2 readers for 2 BDTs
    ...
    TString dir = (string(getenv("CMSSW_BASE"))+"/src/ttHMultileptonAnalysis/TemplateMakers/data/NOVa/Oct31/BDT_weights_ttZ_3l/").c_str();
...
void FinalBDT_ttZ_3l_Oct31::evaluate() {
  if (this->evaluatedThisEvent) return;
  if ((*jets)->size() < 3) return;
  evaluatedThisEvent = true;

  //std::cout << "Inside FinalBDT_ttZ_3l_Oct31::evaluate()" << std::endl;
...
//   std::cout << "varZLike_mass_leplep_SFOS_all: " << varZLike_mass_leplep_SFOS_all << std::endl;
  ...
  for( unsigned int jj = 0 ; jj < 2 ; ++jj ) {
    ...
    if (jj == 0 && (*jets)->size() == 3) branches["FinalBDT_ttZ_3l_Oct31"].branchVal = annOut;
    if (jj == 1 && (*jets)->size() >= 4) branches["FinalBDT_ttZ_3l_Oct31"].branchVal = annOut;
